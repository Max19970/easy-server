...
Local State persists configuration and provider/resource identities, not raw credentials. It may also retain a minimal last-known instance observation containing only normalized state, an optional display name and observation timestamp so degraded inventory remains useful during a Provider outage. Raw Provider state, lifecycle actions, provider error payloads and secret material are not persisted as part of that observation. Writes use temporary files, explicit file creation, fsync and atomic replacement; concurrent writers coordinate through an ownership-generation lock. After the primary state commits, EasyServer atomically advances a validated `state.json.recovery` last-known-good mirror. A missing or corrupt primary can be read from that recovery generation without rotating canonical instance identities or Secret References; if prior state is evident but neither primary nor recovery is valid, EasyServer fails closed instead of silently resetting to an empty installation.
