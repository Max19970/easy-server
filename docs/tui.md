...
Route-specific letter/number shortcuts are not a supported control surface for ordinary tasks: use the visible focused row and **Actions** instead. The optional `r` refresh accelerator is available only where the same visible Refresh action exists, and operation-result accelerators resolve through the same displayed action set. Routine route/focus changes are communicated by the visible location and focus itself; the shell keeps one contextual keyboard hint and reserves status text for feedback that can change the next decision.
