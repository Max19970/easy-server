...
import { hostTrustRequiredError } from "@easycompute/plugin-sdk";
...
async function withStore(run) {
  const directory = await mkdtemp(join(tmpdir(), "easycompute-connect-"));
  try {
...
test("foreground connect exposes a reachable EasyCompute endpoint until cancellation", async () => {
  await withStore(async (store) => {
    ...
    socket.write("through-easycompute");
    assert.equal((await received).toString(), "through-easycompute");
    socket.destroy();
