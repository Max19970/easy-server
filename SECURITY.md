...
While `0.2.x` is the current released minor line, security fixes target the latest released `0.2.x` version. Critical security or data-integrity fixes may still be backported to `0.1.x` at maintainer discretion, but general maintenance and indefinite backport support for the previous line are not promised. Unreleased development snapshots are not a supported release line.
